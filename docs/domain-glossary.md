...
- **已评审是终态（2026-09-14）**：`rollback/`、`PATCH`、附件上传 / 标记 / 删除一律 409 `STAGE_REVIEW_LOCKED`（`assert_not_locked`），免审完成的也一样；评论仍开放。点错了的纠错出口是**裁剪表**：修订里取消勾选已评审的格子 → 签批生效时 `_delete_stage_reviews` 连同轨迹 / 评论 / 附件删掉 → 重新勾选生成新的。裁剪表因此不再拦已评审（`REVIEW_TAILORING_REVIEW_COMPLETED` 已删），改为格子红色警示 + 提交弹窗提示删除条数。手工评审（无模板）已评审后仍可直接删除。
