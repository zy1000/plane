/**
 * 统一处理 release patch 接口返回的错误结构。
 *
 * 后端 ValidationError 失败时 body 形如：
 *   { error: ["不符合状态流转规则"], reasons: ["请填写开始时间", "请先规划工作项"] }
 *
 * 其他场景可能只有 detail / error 字符串。
 */
export type TReleaseUpdateErrorMessage = {
  title: string;
  message: string;
};
...
export const formatReleaseUpdateError = (err: any): TReleaseUpdateErrorMessage => {
  ...
  return {
    title: "Error!",
    message: fallback ?? "Release could not be updated. Please try again.",
  };
};
