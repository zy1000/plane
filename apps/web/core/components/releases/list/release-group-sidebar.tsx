...
export type TReleaseSidebarGroup = {
  id: string;
  name: string;
  count: number;
  icon?: ReactNode;
...
export const ReleaseGroupSidebar = observer(function ReleaseGroupSidebar(props: Props) {
  const { groups, selectedGroupId, onSelectGroup, groupBy } = props;
  const { t } = useTranslation();
  const groupByLabel = groupBy === "status" ? t("state") : groupBy === "lead" ? t("lead") : t("none");
