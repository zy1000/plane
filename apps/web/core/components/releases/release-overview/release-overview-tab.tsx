...
const SECTION_CARD = "rounded-xl border border-subtle bg-surface-1 transition-shadow hover:shadow-sm";
...
export const ReleaseOverviewTab: React.FC<Props> = observer(
  ({
    ...
    const feedItems = useMemo(() => buildReleaseActivityFeedItems(allActivities, comments), [allActivities, comments]);
    ...
    return (
      <div className="flex min-h-[calc(100vh-9rem)] flex-col gap-4">
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-12">
          ...
          <div className={`${SECTION_CARD} p-5 xl:col-span-8`}>
            ...
            <div className="mt-20">
              <div className="mb-2 flex items-center justify-between text-xs text-placeholder">
                <span>工作项状态分布</span>
                <span className="text-secondary tabular-nums">完成度 {progress}%</span>
              </div>
              <div
              ...
              <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs text-secondary">
                {distribution.map((seg) => (
                  <span key={seg.key} className="inline-flex items-center gap-1.5">
                    ...
                    />
                    <span>{seg.label}</span>
                    <span className="text-placeholder tabular-nums">{seg.count}</span>
                  </span>
                ))}
              </div>
            </div>
          ...
          {/* Key facts (4 cols) */}
          <aside className={`${SECTION_CARD} flex flex-col p-5 xl:col-span-4`}>
            <div className="flex items-center gap-2">
            ...
            <dl className="mt-4 space-y-3">
              ...
              <KeyFact
                ...
                value={
                  typeof daysLeft === "number" ? (
                    <span className={`pl-2 ${KEY_FIELD_LABEL_CLASS} text-primary tabular-nums`}>
                      {daysLeft}
              ...
              <KeyFact
                ...
                value={
                  hasRisk ? (
                    <span
                      className={`inline-flex items-center gap-1 pl-2 ${KEY_FIELD_LABEL_CLASS} text-danger-primary`}
                    >
        ...
        <div className={`${SECTION_CARD} flex h-[min(56vh,34rem)] min-h-[20rem] flex-col p-5`}>
          <div className="flex items-center justify-between">
            ...
            <Button variant="link-neutral" className="text-xs" onClick={onEditNote} disabled={isEditDisabled}>
              编辑
            </Button>
          </div>
          {hasNoteContent ? (
            ...
            <div className="mt-3 grid min-h-0 flex-1 place-items-center text-sm text-placeholder">
              {isEditDisabled ? "暂无发布日志" : "暂无发布日志，点击右上角编辑添加。"}
            </div>
          )}
        </div>
...
const HealthMetric: React.FC<{ icon: React.ReactNode; label: string; value: number }> = ({ icon, label, value }) => (
  <div className="flex items-start gap-2.5">
    ...
    <div className="min-w-0">
      ...
      <div className="text-lg leading-tight font-semibold text-primary tabular-nums">{value}</div>
    </div>
  </div>
);
...
const SummaryCard: React.FC<{
  ...
  <div
    className={`${SECTION_CARD} group hover:border-accent-primary/40 cursor-pointer p-5 transition-colors`}
    onClick={onAction}
    role="button"
    tabIndex={0}
    onKeyDown={(e) => {
      ...
      {metrics.map((m) => (
        <div key={m.label} className="flex items-center gap-2.5 rounded-md px-3 py-2.5">
          ...
          <div className="min-w-0">
            ...
            <div className="text-base leading-tight font-semibold text-primary tabular-nums">{m.value}</div>
          </div>
        </div>
      ))}
    </div>
