        ...
        >
          ...
          {!requirement.is_locked && (
            ...
            >
              <MenuRowLabel
                icon={Trash2}
                label={requirement.approved_version !== null ? t("requirement_approval.request_delete") : t("delete")}
                tone="danger"
              />
            </CustomMenu.MenuItem>
          )}
            ...
            return (
              ...
              */}
                ...
                {isFirstRow && (
                  ...
                  >
                    <div className={cn("flex h-full w-full min-w-0 items-center px-page-x", rowStateClass)}>
                ...
                {orderedColumns.flatMap((orderedColumn) => {
                  ...
                  if (form.children.length === 0) {
                    return [
                      <td
                        key={`${form.id}-empty`}
                        className={cn(REQUIREMENT_GRID_BODY_CELL_CLASS, "text-placeholder", groupCellClass)}
                      >
          ...
          {isLoading && !requirements.length ? (
            ...
            <div className="flex min-h-72 items-center justify-center p-6 text-center">
              <div>
                <p className="text-13 font-medium text-primary">
                  {t("workspace_templates.requirement_types.error.title")}
                </p>
            ...
            <div className="flex min-h-72 items-center justify-center p-6 text-center">
              <div>
                ...
                <p className="mt-2 text-13 font-medium text-primary">{t("requirement_grid.data.no_fields")}</p>
                <p className="mt-1 text-12 text-secondary">{t("requirement_grid.data.no_fields_description")}</p>
              </div>
            </div>
                  ...
                  return field.field_type !== "form" || !field.children.length
                    ...
                    : [
                        ...
                        ...field.children.map((child) => (
                          <col key={child.id} style={{ width: getWidth(child.id, REQUIREMENT_GRID_COLUMN_WIDTH) }} />
                        )),
                        ...(showActionGutter
