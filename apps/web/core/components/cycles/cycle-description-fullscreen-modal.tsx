...
export const CycleDescriptionFullscreenModal: FC<Props> = observer(function CycleDescriptionFullscreenModal(
  ...
  const { isOpen, onClose, workspaceSlug, projectId, cycleId, description, canEdit, initialEditing = false } = props;
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      setToast({ type: TOAST_TYPE.ERROR, title: "保存失败", message: "保存迭代描述失败，请稍后重试。" });
    } finally {
      setSaving(false);
    }
    ...
    <Modal
      title={
        <div className="flex min-h-11 items-center gap-2 pr-2">
          <FileText className="h-4 w-4 shrink-0 text-placeholder" />
          <span className="text-base font-medium text-primary">迭代描述</span>
          <span className="text-sm font-normal text-placeholder">迭代目标与范围说明</span>
        </div>
      }
      open={isOpen}
