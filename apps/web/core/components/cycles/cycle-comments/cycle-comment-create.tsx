...
export const CycleCommentCreate = observer(function CycleCommentCreate(props: Props) {
  const {
    workspaceSlug,
    workspaceId,
    projectId,
  ...
  // 跟踪本次编辑期间已经上传到 S3 的 asset_id，提交成功后回填到 cycle_comment_id，
      ...
      try {
        ...
        if (created?.id && assetIds.length > 0) {
          try {
            ...
            console.error("[cycle-comment] bind assets failed", error);
          }
        }
