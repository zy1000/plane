...
import { flattenCommentDescendants } from "./cycle-comment-utils";
...
type Props = {
  ...
  onReply: (data: ReplyPayload) => Promise<TCycleComment | undefined>;
};

export const CycleCommentCard = observer(function CycleCommentCard(props: Props) {
  const {
    comment,
    childrenByParent,
    commentsById,
  ...
  const getCommentActorName = (targetComment: TCycleComment | undefined) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      console.error("[cycle-comment] delete failed", error);
    }
  };
