      ...
      {entries.length > 0 ? (
        <div className="space-y-1">
          {entries.map((entry) => (
            <div
              key={entry.id}
              className="flex items-center gap-3 rounded-md px-2 py-2 text-xs transition-colors hover:bg-layer-1"
            >
              <div className="w-[72px] flex-shrink-0 text-placeholder">{entry.date}</div>
