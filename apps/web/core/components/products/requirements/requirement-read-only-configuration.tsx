...
const STATE_BADGE = "shrink-0 rounded px-1.5 py-0.5 text-11 font-medium";
...
function ReadOnlyFieldRow({ field, isChild = false }: { field: TRequirementField; isChild?: boolean }) {
  ...
  return (
    <li
      className={cn(
        "grid grid-cols-[minmax(0,2fr)_minmax(0,1.6fr)_auto] items-center gap-x-4 px-4 py-2.5 transition-colors hover:bg-layer-2-hover",
        isChild && "pl-11"
      )}
    >
      ...
      <p className="min-w-0 truncate text-12 text-secondary">{fieldSummary(field, t)}</p>
      <span
        className={cn(
          STATE_BADGE,
          field.is_active ? "bg-success-subtle text-success-primary" : "bg-layer-1 text-secondary"
        )}
      >
        {t(
          field.is_active
...
function ReadOnlyNotice({ hint }: { hint: string }) {
  return (
    <p className="mt-3 flex items-start gap-2 rounded-md bg-layer-1 px-3 py-2 text-12 leading-5 text-secondary">
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col bg-surface-1">
      <header className="shrink-0 border-b border-subtle px-5 py-4 md:px-8 md:py-5">
        ...
        <p className="mt-1 text-11 leading-5 text-secondary">{description}</p>
    ...
    >
      {fields.length ? (
        <div className="overflow-hidden rounded-lg border border-subtle bg-layer-1">
          <div className="grid grid-cols-[minmax(0,2fr)_minmax(0,1.6fr)_auto] items-center gap-x-4 px-4 py-2 text-11 font-medium text-secondary">
            <span>{t("workspace_products.requirements.change.field_props.name")}</span>
            <span>{t("workspace_products.requirements.change.field_props.field_type")}</span>
            <span>{t("workspace_products.requirements.change.field_props.is_active")}</span>
          </div>
          <ul className="divide-y divide-subtle border-t border-subtle bg-layer-2">
            {fields.map((field) => (
              <Fragment key={field.id}>
                <ReadOnlyFieldRow field={field} />
                {field.field_type === "form" &&
                  (field.children.length ? (
                    field.children.map((child) => <ReadOnlyFieldRow key={child.id} field={child} isChild />)
                  ) : (
                    <li className="py-2.5 pr-4 pl-11 text-11 text-tertiary">
...
function DefinitionRow({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div className="grid grid-cols-[8rem_minmax(0,1fr)] items-baseline gap-x-4 px-4 py-3">
...
function DefinitionGroup({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="overflow-hidden rounded-lg border border-subtle bg-layer-2">
      <h2 className="bg-layer-1 px-4 py-2 text-11 font-medium text-secondary">{title}</h2>
      <dl className="divide-y divide-subtle border-t border-subtle">{children}</dl>
    </div>
  );
}
...
function MemberTag({ name, avatarUrl }: { name: string; avatarUrl: string | null | undefined }) {
  return (
    <span className="inline-flex h-6 min-w-0 max-w-full items-center gap-1.5 rounded bg-layer-1 px-1.5 text-11 text-primary">
  ...
  return (
    ...
    >
      <div className="grid max-w-3xl gap-5">
        <DefinitionGroup title={t("workspace_products.requirements.configuration.basic")}>
          ...
          <DefinitionRow label={t("workspace_products.requirements.fields.owner")}>
            <MemberTag
              ...
              avatarUrl={requirement.owner_detail.avatar_url}
            />
          </DefinitionRow>
          ...
          <DefinitionRow label={t("workspace_products.requirements.fields.status")}>
            <span className="flex min-w-0 flex-wrap items-center gap-2">
              ...
              {requirement.current_version !== null && (
                <span className="text-11 text-tertiary">
                  {t("workspace_products.requirements.configuration.current_version", {
        ...
        <DefinitionGroup title={t("workspace_products.requirements.configuration.approval")}>
