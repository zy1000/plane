...
function ReadOnlyNotice({ hint }: { hint: string }) {
  return (
    <p className="mt-3 flex max-w-[65ch] items-start gap-2 rounded-md border border-subtle bg-surface-1 px-3 py-2 text-12 leading-5 text-secondary">
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col bg-surface-1">
      ...
      <div className={cn("min-h-0 flex-1 overflow-y-auto px-5 md:px-8", layered ? "pb-8 md:pb-10" : "py-6 md:py-7")}>
        <div className={cn("w-full", layered ? "max-w-[1480px]" : "max-w-5xl")}>{children}</div>
      </div>
    </div>
  );
}
