 ...
 * WORKSPACE_CASE_TEMPLATE_*：读接受 view 或 manage，写只认 manage，Excel 另有
 * import_export（**导入**还要 manage，因为导入本身就是写）。
 *
...
export const useTemplatePermissions = (workspaceSlug: string | undefined): TTemplatePermissions => {
  ...
  return {
    ...
    canAccessTemplates: canViewLibraries || canViewCaseTemplates,
  };
};
