...
export function ProfileWorkloadOverview({ userProfile }: Props) {
  ...
  const stateDistribution = userProfile.state_distribution;
  const totalOpenIssues = stateDistribution.reduce((total, group) => total + group.state_count, 0);
      ...
      <Card variant={ECardVariant.WITHOUT_SHADOW} className="overflow-hidden p-0">
        <div className="grid grid-cols-1 xl:grid-cols-12">
          <section className="p-5 xl:col-span-5 xl:border-r xl:border-subtle">
            <h4 className="text-14 font-medium text-primary">{t("profile.stats.workload_overview.state_title")}</h4>
            <p className="mt-1 text-11 leading-4 text-placeholder">
              {t("profile.stats.workload_overview.state_description")}
            </p>
              ...
              <div className="mt-4 flex items-center gap-6">
                <div className="relative h-[150px] w-[150px] shrink-0">
                  <PieChart
                    ...
                    data={stateDistribution.map((group) => ({
                    ...
                    cells={stateDistribution.map((group) => ({
                      key: group.state_group,
                  ...
                  <div className="pointer-events-none absolute inset-0 grid place-items-center">
                    <div className="flex flex-col items-center">
                      ...
                      <span className="mt-1 text-11 text-placeholder">
                        {t("profile.stats.workload_overview.open_items")}
                      </span>
                    </div>
                  </div>
                </div>
                <div className="min-w-0 flex-1 space-y-3">
                  {stateDistribution.map((group) => (
                    <div key={group.state_group} className="flex items-center justify-between gap-2 text-12">
                      ...
                      <span className="shrink-0 font-medium text-primary tabular-nums">{group.state_count}</span>
                    </div>
                  ))}
                </div>
              </div>
            ) : (
              <div className="grid h-[190px] place-items-center text-12 text-placeholder">
                {t("profile.stats.workload_overview.empty")}
              </div>
            )}
          </section>

          <section className="border-t border-subtle p-5 xl:col-span-4 xl:border-r xl:border-t-0">
          ...
          <section className="border-t border-subtle p-5 xl:col-span-3 xl:border-t-0">
