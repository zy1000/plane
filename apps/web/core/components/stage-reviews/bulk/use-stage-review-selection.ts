...
/**
 * 列表勾选状态。`selectableIds` 是右侧表格里**看得见且能勾**的行（已评审的不算）。
 *
 * 切分组、改筛选、搜索之后，勾着的只留仍然看得见的 —— 批量条上的「已选 N 条」永远和
 * 表格里打勾的行对得上，不会改到看不见的评审。
 */
export const useStageReviewSelection = (selectableIds: string[]) => {
