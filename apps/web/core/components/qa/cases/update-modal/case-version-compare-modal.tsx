...
export function CaseVersionCompareModal({
  ...
  const historyVersionOptions = React.useMemo(() => {
    ...
    return list.map((v) => {
      ...
      const label = createdAt ? `v${Number(v.version) + 1} · ${createdAt}` : `v${Number(v.version) + 1}`;
  ...
  const baseLabel = compareBaseVersion === undefined ? "历史" : `历史 v${Number(compareBaseVersion) + 1}`;
