 ...
 *
 * `changeType` 由调用方（URL query）持有，作为服务端筛选参数传入 —— 千行明细下分段
 * 筛选必须落到查询里，不能在前端过滤当前页。
 */
export const useRequirementChangeRequestDetail = ({
  ...
  }, [changeRequestId, changeType, cursor, perPage, requirementId, workspaceSlug]);
  ...
  useEffect(() => setCursor(undefined), [changeType]);
