...
export const RepositoryModal: React.FC<Props> = ({
  ...
  React.useEffect(() => {
    const ensureSelectedProjectInOptions = async () => {
      ...
      // If we have the project details in initialValues, use them directly
      const projectDetail = initialValues?.project;
      if (
        projectDetail && 
        typeof projectDetail !== "string" && 
        projectDetail.id === currentProjectId
      ) {
        setProjects((prev) => {
          if (prev.some((p) => p.id === currentProjectId)) return prev;
          return [{ 
            id: projectDetail.id, 
            name: projectDetail.name, 
            logo_props: (projectDetail as any).logo_props 
          }, ...prev];
        });
        return;
      }
