...
import { DeleteOutlined, DownloadOutlined, EyeOutlined, UploadOutlined, SearchOutlined } from "@ant-design/icons";
import { MindmapService, type TMindmapAsset } from "@/services/qa/mindmap.service";
...
export default function MindmapPage() {
  ...
  const service = useMemo(() => new MindmapService(), []);
  ...
  const [assets, setAssets] = useState<TMindmapAsset[]>([]);
  ...
  const fetchAssets = useCallback(
    async (page: number = currentPage, size: number = pageSize, filterParams = filters) => {
      ...
      setLoading(true);
      try {
        const response = await service.listMindmapAssets(String(workspaceSlug), String(projectId), {
          page,
          page_size: size,
        ...
        if (safePage !== page) {
          const response2 = await service.listMindmapAssets(String(workspaceSlug), String(projectId), {
            page: safePage,
            page_size: size,
            ...(filterParams.name ? { name__icontains: filterParams.name } : {}),
          });
  ...
  const handlePreview = useCallback(
    async (record: TMindmapAsset) => {
      if (!workspaceSlug || !projectId || !record?.id) return;
      try {
        const signedUrl = await service.getMindmapAssetPresignedURL(
  ...
  const handleDelete = useCallback(
    async (record: TMindmapAsset) => {
      if (!workspaceSlug || !projectId || !record?.id) return;
      try {
        await service.deleteMindmapAsset(String(workspaceSlug), String(record.id), String(projectId));
  ...
  const handleUpload = async (selectedFile?: File | null) => {
    ...
    setUploading(true);
    try {
      await service.uploadMindmapAsset(String(workspaceSlug), String(projectId), targetFile);
  ...
  const getColumnSearchProps = (dataIndex: keyof TMindmapAsset | string): TableColumnType<TMindmapAsset> => ({
  ...
  const handleSearch = (selectedKeys: string[], dataIndex: keyof TMindmapAsset | string, close?: () => void) => {
  ...
  const handleReset = (clearFilters: () => void, dataIndex: keyof TMindmapAsset | string) => {
    clearFilters();
    setSearchText("");
    const nextFilters = { ...filters };
    if (dataIndex === "name") delete nextFilters.name;
  ...
  const columns: TableProps<TMindmapAsset>["columns"] = [
    {
      title: "文件名",
    ...
    },
    {
      ...
      key: "actions",
      width: 180,
      render: (_: any, record: TMindmapAsset) => (
        <div className="flex items-center">
          ...
          <Tooltip title="下载">
            <Button
              ...
              onClick={() => {
                if (!workspaceSlug || !projectId || !record?.id) return;
                const url = service.getMindmapAssetDownloadURL(
                  String(workspaceSlug),
                  String(record.id),
                  String(projectId)
                );
      ...
      <div className="h-full w-full overflow-hidden flex flex-col">
        <div className="px-3 pt-2 pb-2 flex items-center justify-between flex-shrink-0 border-b border-custom-border-200">
          <Breadcrumbs>
            ...
            />
            <Breadcrumbs.Item isLast component={<Breadcrumbs.ItemWrapper>文件存储</Breadcrumbs.ItemWrapper>} />
          </Breadcrumbs>
