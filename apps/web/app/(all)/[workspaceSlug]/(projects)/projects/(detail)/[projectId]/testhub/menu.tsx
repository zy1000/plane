...
const MENU_ITEMS: TMenuItem[] = [
  ...
  },
  {
    key: "cases",
    label: "测试用例",
    href: (ws, pid) => `/${ws}/projects/${pid}/testhub/cases`,
    isActive: (pathname, ws, pid) => pathname.startsWith(`/${ws}/projects/${pid}/testhub/cases`),
  },
  {
    key: "plans",
    label: "测试计划",
    href: (ws, pid) => `/${ws}/projects/${pid}/testhub/plans`,
    isActive: (pathname, ws, pid) => pathname.startsWith(`/${ws}/projects/${pid}/testhub/plans`),
  },
  {
    key: "reviews",
    label: "用例评审",
    href: (ws, pid) => `/${ws}/projects/${pid}/testhub/reviews`,
    isActive: (pathname, ws, pid) => pathname.startsWith(`/${ws}/projects/${pid}/testhub/reviews`),
  },
];
  ...
  const handleRepositoryChange = React.useCallback(
    (repo: { id: string | null; name?: string | null }) => {
      ...
      if (repo.id) {
        ...
        router.push(`/${ws}/projects/${pid}/testhub/plans?repositoryId=${encodeURIComponent(String(repo.id))}`);
      } else {
