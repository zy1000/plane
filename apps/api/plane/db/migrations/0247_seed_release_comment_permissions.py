...
RELEASE_COMMENT_PERMISSIONS = [
    {
        "key": "releases.comment.create",
        "name": "创建发布评论",
        "action": "create",
        "sort_order": 10,
    },
...
def seed_release_comment_permissions(apps, schema_editor):
    """新增三条 releases.comment.* 权限并补齐到已有的 ProjectRole 中。"""
    Permission = apps.get_model("db", "Permission")
    ProjectRole = apps.get_model("db", "ProjectRole")

    new_keys = []
    for permission in RELEASE_COMMENT_PERMISSIONS:
    ...
    # 把新增 key 补齐到已有的、本来就拥有 releases.* 权限的 ProjectRole 中，
    # 让既有项目成员可以直接使用评论能力。
...
def unseed_release_comment_permissions(apps, schema_editor):
    """反向：移除三条权限定义，并把 ProjectRole 中的对应 key 清掉。"""
    Permission = apps.get_model("db", "Permission")
    ProjectRole = apps.get_model("db", "ProjectRole")

    keys_to_remove = {permission["key"] for permission in RELEASE_COMMENT_PERMISSIONS}
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("db", "0246_release_comment"),
    ]

    operations = [
        migrations.RunPython(
            seed_release_comment_permissions, unseed_release_comment_permissions
        ),
    ]
