...
def create_release_issue_activity(
    ...
    issue_activities.append(
        IssueActivity(
            ...
            project_id=project_id,
            workspace_id=workspace_id,
            comment=f"added release {release.name if release else ''}",
...
def delete_release_issue_activity(
    ...
    requested_data = json.loads(requested_data) if requested_data is not None else None
    current_instance = json.loads(current_instance) if current_instance is not None else None
    release_name = current_instance.get("release_name") if current_instance else ""
    current_issue = Issue.objects.filter(pk=issue_id).first()
    if current_issue:
        current_issue.updated_at = timezone.now()
        current_issue.save(update_fields=["updated_at"])
    issue_activities.append(
        IssueActivity(
            ...
            project_id=project_id,
            workspace_id=workspace_id,
            comment=f"removed this issue from {release_name}",
            old_identifier=(requested_data.get("release_id") if requested_data.get("release_id") is not None else None),
            epoch=epoch,
        )
    )
